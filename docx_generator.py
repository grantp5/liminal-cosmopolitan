...
def create_docx(final_order_tu, final_order_b, name, paired_questions):
    doc = docx.Document()
    ...
    if paired_questions == 1:
        for x in final_order_tu:
            ...
            for y in range(3):
                ...
                d.underline = True
            d = doc.add_paragraph('<')
            d.add_run(final_order_b[final_order_tu.index(x)])
            d.add_run('>')
            doc.add_paragraph()
            doc.add_paragraph()

    if paired_questions == 0:
        for x in final_order_tu:
            number = final_order_tu.index(x ) +1
            a = doc.add_paragraph(str(number))
            a.add_run('. ')
